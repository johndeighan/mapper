// Generated by CoffeeScript 2.5.1
// StringInput.test.coffee
var GatherTester, dir, root, simple, tester;

import {
  strict as assert
} from 'assert';

import {
  resolve
} from 'path';

import {
  say,
  undef,
  pass,
  isEmpty,
  setUnitTesting,
  unitTesting,
  escapeStr
} from '@jdeighan/coffee-utils';

import {
  indentLevel,
  undented,
  splitLine,
  indented
} from '@jdeighan/coffee-utils/indent';

import {
  debug,
  debugging
} from '@jdeighan/coffee-utils/debug';

import {
  mydir,
  mkpath
} from '@jdeighan/coffee-utils/fs';

import {
  UnitTester
} from '@jdeighan/coffee-utils/test';

import {
  StringInput
} from '@jdeighan/string-input';

dir = mydir(import.meta.url);

root = resolve(dir, '..');

process.env.DIR_MARKDOWN = mkpath(root, 'src', 'markdown');

simple = new UnitTester();

setUnitTesting(true);

// ---------------------------------------------------------------------------
(function() {
  var input, item;
  input = new StringInput(`abc
def
ghi`);
  item = input.peek();
  simple.equal(40, item, 'abc');
  item = input.peek();
  simple.equal(42, item, 'abc');
  item = input.get();
  simple.equal(44, item, 'abc');
  item = input.get();
  simple.equal(46, item, 'def');
  input.unget(item);
  item = input.get();
  simple.equal(49, item, 'def');
  input.skip();
  item = input.get();
  return simple.equal(52, item, undef);
})();

// ---------------------------------------------------------------------------
GatherTester = class GatherTester extends UnitTester {
  transformValue(oInput) {
    assert(oInput instanceof StringInput, "oInput should be a StringInput object");
    return oInput.getAll();
  }

};

tester = new GatherTester();

// ---------------------------------------------------------------------------
// --- Test basic reading till EOF
tester.equal(70, new StringInput(`abc
def`), ['abc', 'def']);

tester.equal(78, new StringInput(`abc

def`), ['abc', '', 'def']);

(function() {
  var TestInput;
  TestInput = class TestInput extends StringInput {
    mapLine(line) {
      if (line === '') {
        return undef;
      } else {
        return line;
      }
    }

  };
  return tester.equal(97, new TestInput(`abc

def`), ['abc', 'def']);
})();

// ---------------------------------------------------------------------------
// --- Test basic use of mapping function
(function() {
  var TestInput;
  TestInput = class TestInput extends StringInput {
    mapLine(line) {
      if (line === '') {
        return undef;
      } else {
        return 'x';
      }
    }

  };
  return tester.equal(119, new TestInput(`abc

def`), ['x', 'x']);
})();

// ---------------------------------------------------------------------------
// --- Test ability to access 'this' object from a mapper
//     Goal: remove not only blank lines, but also the line following
(function() {
  var TestInput;
  TestInput = class TestInput extends StringInput {
    mapLine(line) {
      var follow;
      if (line === '') {
        follow = this.fetch();
        return undef;
      } else {
        return line;
      }
    }

  };
  return tester.equal(144, new TestInput(`abc

def
ghi`), ['abc', 'ghi']);
})();

// ---------------------------------------------------------------------------

  // --- Test mapping to objects
(function() {
  var TestInput, cmdRE;
  cmdRE = /^\s*\#([a-z][a-z_]*)\s*(.*)$/; // skip leading whitespace
  // command name
  // skipwhitespace following command
  // command arguments
  TestInput = class TestInput extends StringInput {
    mapLine(line) {
      var lMatches;
      lMatches = line.match(cmdRE);
      if (lMatches != null) {
        return {
          cmd: lMatches[1],
          argstr: lMatches[2]
        };
      } else {
        return line;
      }
    }

  };
  return tester.equal(177, new TestInput(`abc
#if x==y
	def
#else
	ghi`), [
    'abc',
    {
      cmd: 'if',
      argstr: 'x==y'
    },
    '\tdef',
    {
      cmd: 'else',
      argstr: ''
    },
    '\tghi'
  ]);
})();

// ---------------------------------------------------------------------------
// --- Test continuation lines
(function() {
  var TestInput;
  TestInput = class TestInput extends StringInput {
    mapLine(line) {
      var n, next;
      if (line === '' || line.match(/^\s*#\s/)) {
        return undef; // skip comments and blank lines
      }
      n = indentLevel(line); // current line indent
      while ((this.lBuffer.length > 0) && (indentLevel(this.lBuffer[0]) >= n + 2)) {
        next = this.lBuffer.shift();
        line += ' ' + undented(next);
      }
      return line;
    }

  };
  return tester.equal(209, new TestInput(`str = compare(
		"abcde",
		expected
		)

call func
		with multiple
		long parameters

# --- DONE ---`), ['str = compare( "abcde", expected )', 'call func with multiple long parameters']);
})();

// ---------------------------------------------------------------------------
// --- Test continuation lines AND HEREDOCs
(function() {
  var TestInput;
  TestInput = class TestInput extends StringInput {
    mapLine(line) {
      var n, next;
      if (line === '' || line.match(/^\s*#\s/)) {
        return undef; // skip comments and blank lines
      }
      n = indentLevel(line); // current line indent
      while ((this.lBuffer.length > 0) && (indentLevel(this.lBuffer[0]) >= n + 2)) {
        next = this.lBuffer.shift();
        line += ' ' + undented(next);
      }
      return line;
    }

  };
  return tester.equal(244, new TestInput(`str = compare(
		"abcde",
		expected
		)

call func
		with multiple
		long parameters

# --- DONE ---`), ['str = compare( "abcde", expected )', 'call func with multiple long parameters']);
})();

// ---------------------------------------------------------------------------
// --- Test overriding the class
(function() {
  var TestInput;
  TestInput = class TestInput extends StringInput {
    mapLine(line) {
      if (isEmpty(line)) {
        return undef;
      }
      if (line === 'abc') {
        return '123';
      } else if (line === 'def') {
        return '456';
      } else {
        return line;
      }
    }

  };
  return tester.equal(280, new TestInput(`abc

def`), ['123', '456']);
})();

// ---------------------------------------------------------------------------
// --- Test #include
tester.equal(294, new StringInput(`abc
	#include title.md
def`), ['abc', '\tContents of title.md', 'def']);

// ---------------------------------------------------------------------------
// --- Test #include with unit testing off
setUnitTesting(false);

tester.equal(308, new StringInput(`abc
	#include title.md
def`), ['abc', '\ttitle', '\t=====', 'def']);

setUnitTesting(true);

// ---------------------------------------------------------------------------
// --- Test advanced use of mapping function
(function() {
  var TestInput;
  TestInput = class TestInput extends StringInput {
    mapLine(orgLine) {
      var lMatches, level, line, result;
      [level, line] = splitLine(orgLine);
      if (isEmpty(line) || line.match(/^#\s/)) {
        return undef;
      }
      if (lMatches = line.match(/^(?:([A-Za-z][A-Za-z0-9_]*)\s*)?\<\=\=\s*(.*)$/)) { // variable name
        result = indented(line, level);
      } else {
        result = orgLine;
      }
      return result;
    }

  };
  return tester.equal(344, new TestInput(`\tabc
\t	myvar <== 2 * 3

\tdef`), ['\tabc', '\t\tmyvar <== 2 * 3', '\tdef']);
})();

// ---------------------------------------------------------------------------
// --- Test #include inside block processed by fetchBlock()
(function() {
  var TestParser, block, line, oInput, text;
  text = `p a paragraph
div:markdown
	#include title.md`;
  block = undef;
  TestParser = class TestParser extends StringInput {
    mapLine(line) {
      if (line === 'div:markdown') {
        block = this.fetchBlock(1);
      }
      return line;
    }

  };
  oInput = new TestParser(text);
  line = oInput.get();
  simple.equal(376, line, 'p a paragraph');
  line = oInput.get();
  simple.equal(378, line, 'div:markdown');
  return simple.equal(379, block, 'Contents of title.md');
})();

// ---------------------------------------------------------------------------
// --- Test blank lines inside a block
(function() {
  var TestParser, block, line, oInput, text;
  text = `p a paragraph
div:markdown
	line 1

	line 3`;
  block = undef;
  TestParser = class TestParser extends StringInput {
    mapLine(line) {
      if (line === 'div:markdown') {
        block = this.fetchBlock(1);
      }
      return line;
    }

  };
  oInput = new TestParser(text);
  line = oInput.get();
  simple.equal(404, line, 'p a paragraph');
  line = oInput.get();
  simple.equal(406, line, 'div:markdown');
  return simple.equal(407, block, `line 1

line 3`);
})();

// ---------------------------------------------------------------------------
(function() {
  var TestParser, block, line, oInput, text;
  text = `p a paragraph
div:markdown
	#include title.md`;
  block = undef;
  TestParser = class TestParser extends StringInput {
    mapLine(line) {
      if (line === 'div:markdown') {
        block = this.fetchBlock(1);
      }
      return line;
    }

  };
  setUnitTesting(false);
  oInput = new TestParser(text);
  line = oInput.get();
  simple.equal(433, line, 'p a paragraph');
  line = oInput.get();
  simple.equal(436, line, 'div:markdown');
  simple.equal(438, block, '\ttitle\n\t=====');
  return setUnitTesting(true);
})();

// ---------------------------------------------------------------------------
(function() {
  /* Contents of files used:

  ```header.md
  header
  ======

  	#include para.md
  ```

  ```para.md
  para
  ----
  ```
   */
  var oInput, text;
  text = `p a paragraph
div:markdown
	#include header.md`;
  oInput = new StringInput(text);
  setUnitTesting(false);
  tester.equal(470, oInput, ["p a paragraph", "div:markdown", "\theader", "\t======", "", "\t\tpara", "\t\t----"]);
  return setUnitTesting(true);
})();

// ---------------------------------------------------------------------------
// --- Test comment
tester.equal(294, new StringInput(`abc

# --- this is a comment

def`), ['abc', '', '# --- this is a comment', '', 'def']);
