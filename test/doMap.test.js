...
export var JSMapper = class JSMapper extends Mapper {
  mapComment(hNode) {
    var level, str;
    ({str, level} = hNode);
  ...
  mapCmd(hNode) {
    ...
    if (cmd === 'reactive') {
      if (argstr === '') {
        ...
        code = doMap(JSMapper, this.source, code);
        block = arrayToBlock(["# |||| $: {", code, "# |||| }"]);
      } else {
        // --- simulate conversion to JavaScript
        code = doMap(JSMapper, this.source, argstr);
        block = arrayToBlock(["# |||| $:", code]);
      }
...
(function() {
  ...
  tester.equal(137, `# --- a comment (should remove)
...
<script>
  ...
  return tester.equal(152, `# --- a comment (should remove)
...
<script>
  ...
  return tester.equal(221, `<h1>title</h1>
<script>
	# |||| $: {
	y = 2*x
	console.log "OK"
...
(function() {
  ...
  tester.equal(256, `# --- a comment (should remove)
...
<script>
  ...
  return tester.equal(271, `# --- a comment (should remove)
